...
titleSuffix: Azure SQL Database & SQL database in Microsoft Fabric
...
You can query dynamic management views (DMVs) via Transact-SQL (T-SQL) to monitor workload performance and diagnose performance problems, which might be caused by blocked or long-running queries, resource bottlenecks, suboptimal query plans, and more.
...
The `sys.dm_db_resource_stats` view shows recent resource use data relative to the limits of the compute size. Percentages of CPU, data I/O, log writes, worker threads, and memory usage toward the limit are recorded for every 15-second interval and are maintained for approximately one hour.

For other sample queries, see the examples in [sys.dm_db_resource_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database?view=azuresqldb-current&preserve-view=true).
...
:::image type="content" source="media/monitoring-with-dmvs/sql-db-resource-utilization.png" alt-text="Screenshot of a sample graph of database resource use.":::
...
1. Once you identify the problematic queries, it's time to tune those queries to reduce CPU utilization. Alternatively, you can choose to increase the compute size of the database or elastic pool to work around the issue.

For more information about handling CPU performance problems in Azure SQL Database, see [Diagnose and troubleshoot high CPU on Azure SQL Database and SQL database in Microsoft Fabric](high-cpu-diagnose-troubleshoot.md).
...
- `PAGEIOLATCH_*`
  ...
  For data file I/O issues (including `PAGEIOLATCH_SH`, `PAGEIOLATCH_EX`, `PAGEIOLATCH_UP`). If the wait type name has **IO** in it, it points to an I/O issue. If there's no **IO** in the page latch wait name, it points to a different type of problem that isn't related to storage performance (for example, `tempdb` contention).
...
If your top wait type is `RESOURCE_SEMAPHORE`, you might have a memory grant waiting issue where queries can't start executing until they get a sufficiently large memory grant.
...
If you encounter out of memory errors in Azure SQL Database, review [sys.dm_os_out_of_memory_events](/sql/relational-databases/system-dynamic-management-views/sys-dm-os-out-of-memory-events?view=azuresqldb-current&preserve-view=true). For more information, see [Troubleshoot out of memory errors with Azure SQL Database and Fabric SQL database](troubleshoot-memory-errors-issues.md).
...
Slow or long running queries can consume significant system resources. This section demonstrates how to use dynamic management views to detect a few common query performance problems using the [sys.dm_exec_query_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-query-stats-transact-sql?view=azuresqldb-current&preserve-view=true) dynamic management view. The view contains one row per query statement within the cached plan, and the lifetime of the rows are tied to the plan itself. When a plan is removed from the cache, the corresponding rows are eliminated from this view. If a query doesn't have a cached plan, for example because `OPTION (RECOMPILE)` is used, it's not present in the results from this view.
...
### Monitor query plans for cumulative CPU time

An inefficient query plan also might increase CPU consumption. The following example determines which query uses the most cumulative CPU in recent history.
...
- [Analyze and prevent deadlocks in Azure SQL Database and SQL database in Fabric](analyze-prevent-deadlocks.md)
