...
You can ingest, process, view, and analyze real-time streaming data into a table directly from a database in Azure SQL Database using [Azure Stream Analytics](/azure/stream-analytics/stream-analytics-introduction). Azure Stream Analytics provides a wide variety of scenarios such as connected car, remote monitoring, fraud detection, and many more. 

In the Azure portal, you can select an events source (Event Hub/IoT Hub), view incoming real-time events, and select a table to store events. You can also write Azure Stream Analytics Query Language queries in the portal to transform incoming events and store them in the selected table. This entry point is in addition to the creation and configuration experiences that already exist in Stream Analytics. This experience starts from the context of your database, enabling you to quickly set up a Stream Analytics job and navigate seamlessly between the database in Azure SQL Database and Stream Analytics experiences.

:::image type="content" source="media/stream-data-stream-analytics-integration/stream-analytics-flow.png" alt-text="Stream Analytics flow." lightbox="media/stream-data-stream-analytics-integration/stream-analytics-flow.png":::
...
1. Navigate to the database where you want to ingest your streaming data. Under **Integrations**, select **Stream analytics (preview)**.
1. To start ingesting your streaming data into this database, select **Create** and give a name to your streaming job, and then select **Next: Input**.

    :::image type="content" source="media/stream-data-stream-analytics-integration/create-job.png" alt-text="Screenshot from the Azure portal showing how to configure a new Stream Analytics job basics." lightbox="media/stream-data-stream-analytics-integration/create-job.png":::

1. Enter your events source details, and then select **Next: Output**.
   ...
   - **Event Hub consumer group** (Default to create new): Give a consumer group name  
      ...
      We recommend that you create a consumer group and a policy for each new Azure Stream Analytics job that you create from here. Consumer groups allow only five concurrent readers, so providing a dedicated consumer group for each job will avoid any errors that might arise from exceeding that limit. A dedicated policy allows you to rotate your key or revoke permissions without affecting other resources.

     :::image type="content" source="media/stream-data-stream-analytics-integration/create-job-output.png" alt-text="Screenshot from the Azure portal showing the  configure Stream Analytics job output wizard." lightbox="media/stream-data-stream-analytics-integration/create-job-output.png":::
...
1. Select which table you want to ingest your streaming data into. Once done, select **Create**.
   ...
   - **Table**: Select **Create new** or **Use existing**. In this flow, let's select **Create**. This will create a new table when you start the stream Analytics job.
     ...
     :::image type="content" source="media/stream-data-stream-analytics-integration/create.png" alt-text="Screenshot from the Azure portal showing the create Stream Analytics job button." lightbox="media/stream-data-stream-analytics-integration/create.png":::
...
1. A query page opens with following details:
   ...
   - **Input preview**: Shows snapshot of latest incoming data from input events source.
     ...
     :::image type="content" source="media/stream-data-stream-analytics-integration/test-query.png" alt-text="Test query." lightbox="media/stream-data-stream-analytics-integration/test-query.png":::

   - **Test results**: Select **Test query** and you can see the results of your streaming query

     :::image type="content" source="media/stream-data-stream-analytics-integration/test-results.png" alt-text="Test results." lightbox="media/stream-data-stream-analytics-integration/test-results.png":::
   ...
   - **Test results schema**: Shows the schema of the results of your streaming query after testing. Make sure the test results schema matches with your output schema.
     ...
     :::image type="content" source="media/stream-data-stream-analytics-integration/test-results-schema.png" alt-text="Test results schema." lightbox="media/stream-data-stream-analytics-integration/test-results-schema.png":::
...
1. After you're done authoring & testing the query, select **Save query**. Select **Start Stream Analytics job** to start ingesting transformed data into the SQL table. Once you finalize the following fields, **start** the job.
   ...
   - **Max batch count**: The recommended upper limit on the number of records sent with every bulk insert transaction.  
    For more information about output error handling, see [Output error policies in Azure Stream Analytics](/azure/stream-analytics/stream-analytics-output-error-policy).  
     ...
     :::image type="content" source="media/stream-data-stream-analytics-integration/start-job.png" alt-text="Screenshot from the Azure portal showing how to start a job." lightbox="media/stream-data-stream-analytics-integration/start-job.png":::

1. Once you start the job, you'll see the Running job in the list, and you can take following actions:
   ...
   - **Open in Stream Analytics**: Open the job in Stream Analytics to view monitoring, debugging details of the job.
     ...
     :::image type="content" source="media/stream-data-stream-analytics-integration/jobs.png" alt-text="Screenshot from the Azure portal showing Stream Analytics jobs." lightbox="media/stream-data-stream-analytics-integration/jobs.png":::
