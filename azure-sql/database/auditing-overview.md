...
- Due to performance constraints, we do not audit **tempdb** and **temporary tables**. While the batch completed action group captures statements against temporary tables, it may not correctly populate the object names. However, the source table is always audited, ensuring that all inserts from the source table to temporary tables are recorded.
