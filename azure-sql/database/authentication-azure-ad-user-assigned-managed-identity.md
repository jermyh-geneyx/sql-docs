...
Alternatively, *only on Azure SQL Database*, using a SMI gives an opportunity to not explicitly provision the Graph permissions. The Entra users can still be created without the needed Graph permission by using the below `CREATE USER` T-SQL syntax. This would require the `SID` and `TYPE` syntax, as described in the article, [CREATE USER (Transact-SQL)](/sql/t-sql/statements/create-user-transact-sql#syntax).
