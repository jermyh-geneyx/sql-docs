...
To monitor operations for a Hyperscale database, first connect to the `master` database on your [logical server](logical-servers.md) using [SQL Server Management Studio (SSMS)](/ssms/sql-server-management-studio-ssms) or the client of your choice to run Transact-SQL commands.
...
To review the service tiers of all Hyperscale databases on a [logical server](logical-servers.md) with Transact-SQL, first connect to the `master` database using [SQL Server Management Studio (SSMS)](/ssms/sql-server-management-studio-ssms).
