...
ms.author: wiassaf
ms.reviewer: nnikolic, mathoma, derekw, randolphwest
ms.date: 04/23/2025
...
The following solution automates the sending of email notifications containing automatic tuning recommendations. The solution described consists of automating execution of a PowerShell script for retrieving tuning recommendations using [Azure Automation](/azure/automation/automation-intro), and automation of scheduling email delivery job using [Microsoft Power Automate](https://make.powerautomate.com).
...
1. Select **+ Create a resource** in the resource menu.
1. Search for **Automation**.
1. Select the Automation app in the search results.

   :::image type="content" source="media/automatic-tuning-email-notifications-configure/marketplace-automation.png" alt-text="Screenshot from the Azure portal searching for Automation.":::

1. Once inside the "Create an Automation Account" pane, select **Create**.
1. Select a subscription and resource group to be used for the PowerShell script execution. Enter a name for this automation account.
1. Select **Advanced**. Verify that **System assigned** is checked, so that a system assigned managed identity (SAMI) will be created to authenticate to Azure resources.
...
- To update your Azure modules, see [Az module support in Azure Automation](/azure/automation/shared-resources/modules).
...
1. In the first line of the "Edit PowerShell Runbook" (starting with the number 1), copy-paste the following PowerShell script code. This PowerShell script is provided as-is to get you started. Modify the script to suite your needs.
...
Select **Save** to save the script. When you're satisfied with the script, and select **Publish** to publish this runbook.

At the main runbook pane, you can choose to select **Start** to **test** the script. Select **Output** to view results of the script executed. This output is going to be the content of your email.
...
Prerequisite for this step is to sign up for a [Microsoft Power Automate](https://make.powerautomate.com) account and to sign in. Once inside the solution, follow these steps to set up a **new flow**:

1. Access **My flows** menu item.
1. Inside My flows, select the **+Create from blank** link at the top of the page.
1. Select the link the **Search for hundreds of connectors and triggers**.
...
The previous steps configure the email delivery job workflow. The entire flow consisting of three actions built is shown in the following image.

To test the flow, select **Run Now**. Statistics of running the automated jobs, showing success of email notifications sent out, can be seen from the Flow analytics pane.

The Flow analytics pane is helpful for monitoring the success of job executions, and if necessary for troubleshooting. In the case of troubleshooting, you also might want to examine the PowerShell script execution log accessible through the Azure Automation app.
...
:::image type="content" source="media/automatic-tuning-email-notifications-configure/automatic-tuning-email.png" alt-text="Screenshot from Microsoft Outlook of a sample email output from automatic tuning email notifications." lightbox="media/automatic-tuning-email-notifications-configure/automatic-tuning-email.png":::
...
- [Monitor queries and improve workload performance with automatic tuning in the Azure portal](automatic-tuning-enable.md)
- [Find and apply performance recommendations](database-advisor-find-recommendations-portal.md)
