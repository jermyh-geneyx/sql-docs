...
This article gives an overview of how to set up infrastructure and SQL managed instances to implement [Windows Authentication for principals on Azure SQL Managed Instance](winauth-azuread-overview.md) with Microsoft Entra ID ([formerly Azure Active Directory](/entra/fundamentals/new-name)).
...
- **Configuration of Azure SQL Managed Instance.**
  - Create a system assigned service principal for each SQL managed instance.
...
Following this, a system administrator configures authentication flows. Two authentication flows are available to implement Windows Authentication for Microsoft Entra principals on Azure SQL Managed Instance:
- The incoming trust-based flow supports AD joined clients running Windows server 2012 or higher.
- The modern interactive flow supports Microsoft Entra joined clients running Windows 10 21H1 or higher.

<a id="synchronize-ad-with-azure-ad"></a>
...
### Select which authentication flows you'll implement
...
|Application must connect to the SQL managed instance via an interactive session. | This supports applications such as SQL Server Management Studio (SSMS) and web applications but won't work for applications that run as a service. |
...
### Prerequisites to configure a SQL managed instance
...
See [Configure Azure SQL Managed Instance for Windows Authentication for Microsoft Entra ID](winauth-azuread-kerberos-managed-instance.md) for steps to configure each SQL managed instance.
...
Windows limits how often it connects to Microsoft Entra ID, so there's a potential for user accounts to not have a refreshed Kerberos Ticket Granting Ticket (TGT) within 4 hours of an upgrade or fresh deployment of a client machine.  User accounts who don't have a refreshed TGT results in failed ticket requests from Microsoft Entra ID.
