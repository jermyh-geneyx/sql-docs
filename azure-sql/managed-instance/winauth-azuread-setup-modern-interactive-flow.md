...
There's no Active Directory to Microsoft Entra ID set up required to enable running software on Microsoft Entra-joined virtual machines to access Azure SQL Managed Instance using Windows Authentication. The following prerequisites are required to implement the modern interactive authentication flow:
...
|Applications must connect to the SQL managed instance via an interactive session. | This supports applications such as SQL Server Management Studio (SSMS) and web applications, but doesn't work for applications that run as a service. |
...
Users with existing logon sessions might need to refresh their Microsoft Entra [Primary Refresh Token (PRT)](/azure/active-directory/devices/concept-primary-refresh-token) if they attempt to use this feature immediately after enabling it. It can take up to a few hours for the PRT to refresh on its own.
