...
- A network admin needs to allow traffic to the SQL managed instance using a network security group (NSG). For more information, review [network security group permissions](/azure/virtual-network/manage-network-security-group#permissions).
...
1. Open the resource group with the SQL managed instance, and select the **SQL managed instance** that you want to configure public endpoint on.
...
To enable the public endpoint with PowerShell, set `-PublicDataEndpointEnabled` to `true` when you update instance properties with [Set-AzSqlInstance](/powershell/module/az.sql/set-azsqlinstance).
...
To enable the public endpoint with the Azure CLI,  set `--public-data-endpoint-enabled` to `true` when you update instance properties with [az sql mi update](/cli/azure/sql/mi#az-sql-mi-update).
...
To disable the public endpoint with PowerShell, set `-PublicDataEndpointEnabled` to `false` when you update instance properties with [Set-AzSqlInstance](/powershell/module/az.sql/set-azsqlinstance).
...
To disable the public endpoint with the Azure CLI,  set `--public-data-endpoint-enabled` to `false` when you update instance properties with [az sql mi update](/cli/azure/sql/mi#az-sql-mi-update).
...
1. Select the **Subnets** tab on the configuration pane of your Virtual network, and make note of the **SECURITY GROUP** name for your SQL managed instance.
...
1. Go back to the resource group that contains your SQL managed instance. You should see the **Network security group** name noted previously. Select the **Network security group** name to open the **Network Security Group** configuration page.
...
> [Use Azure SQL Managed Instance securely with public endpoints](public-endpoint-overview.md)
