...
The ID of the trace to be modified. *@traceid* is **int**, with no default. The user employs this *@traceid* value to identify, modify, and control the trace.
...
The ID of the column to be added for the event. *@columnid* is **int**, with no default.
...
| `16` | `Reads` | Number of logical disk reads performed by the server on behalf of the event. This column isn't populated by the `Lock:Released` event. |
...
| `21` | `EventSubClass` | Type of event subclass. This data column isn't populated for all event classes. |
| `22` | `ObjectID` | System-assigned ID of the object. |
| `23` | `Success` | Success of the permissions usage attempt; used for auditing.<br /><br />`1` = success<br />`0` = failure |
