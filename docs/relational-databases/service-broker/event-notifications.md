...
description: Event notifications send information about events to a Service Broker service.
author: "MashaMSFT"
ms.author: "mathoma"
...
Event information is delivered to the [!INCLUDE [ssSB](../../includes/sssb-md.md)] service as a variable of type **xml** that provides information about when an event occurs, about the database object affected, the [!INCLUDE [tsql](../../includes/tsql-md.md)] batch statement involved, and other information. For more information about the XML schema produced by event notifications, see [EVENTDATA](../../t-sql/functions/eventdata-transact-sql.md).
...
| Triggers can run Transact-SQL or common language runtime (CLR) managed code. | Event notifications don't run code. Instead, they send **xml** messages to a Service Broker service. |
| Triggers are processed synchronously, within the scope of the transactions that cause them to fire. | Event notifications might be processed asynchronously and don't run in the scope of the transactions that cause them to fire. |
...
| Triggers can be rolled back. | Event notifications can't be rolled back. |
...
| DML triggers are owned by the same owner as the tables on which they are applied. | The owner of an event notification on a queue might have a different owner than the object on which it's applied. |
| Triggers support the `EXECUTE AS` clause. | Event notifications don't support the `EXECUTE AS` clause. |
...
| --- | --- |
| SQL Trace generates no performance overhead associated with transactions. Packaging of data is efficient. | There's performance overhead associated with creating the XML-formatted event data and sending the event notification. |
...
| Trace events generated by DDL are always generated, regardless of whether the DDL statement is rolled back. | Event notifications don't fire if the event in the corresponding DDL statement is rolled back. |
...
| After being initiated, the firing of traces can't be controlled. Stop times and filter times can be used to specify when they initiate. Traces are accessed by polling the corresponding trace file. | Event notifications can be controlled by using the `WAITFOR` statement against the queue that receives the message generated by the event notification. They can be accessed by polling the queue. |
