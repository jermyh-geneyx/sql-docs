...
> [!NOTE]  
> For more information on SQL Server backup to Azure Blob Storage, see [SQL Server backup and restore with Azure Blob Storage](sql-server-backup-and-restore-with-microsoft-azure-blob-storage-service.md).
...
<a id="OvMediaSetsFamiliesBackupSets"></a>
...
Compressed and uncompressed backups can't occur together in a media set. Any edition of [!INCLUDE [sql2008-md](../../includes/sql2008-md.md)] or later can read compressed backups. For more information, see [Backup compression (SQL Server)](backup-compression-sql-server.md).
...
Each tape or disk in a media family is assigned a *media sequence number*. The media sequence number of a disk is always 1. In a tape media family, the sequence number of the initial tape is 1, the sequence number of the second tape is 2, and so forth.
...
:::image type="content" source="media/media-sets-media-families-and-backup-sets-sql-server/media-set-new.png" alt-text="Diagram of media header and first backup set on three tapes.":::
...
:::image type="content" source="media/media-sets-media-families-and-backup-sets-sql-server/media-set-appended-to.png" alt-text="Diagram of second backup set spread across three media-set tapes.":::
...
1. Writes a new media header on the backup media (tape or disk file) on each of the backup devices.

<a id="UseExistingMediaSet"></a>

## Back up to an existing media set

When you're backing up to an existing media set, you have the following two options:

- Append to the existing backup set.

  To make the best possible use of the available space, new backup sets typically are appended to existing media set. Appending to the backup preserves any prior backups. For more information, see [Append to existing backup sets](#append-to-existing-backup-sets), later in this section.
...
- Overwrite all existing backup sets with the current backup, leaving the current media header in place.
  ...
  For tape headers, leaving the header in place can make sense. For more information, see [Overwrite backup sets](#overwrite-backup-sets), later in this section.

  Overwriting existing backup sets is specified by using the `INIT` option of the `BACKUP` statement.

<a id="Appending"></a>

## Append to existing backup sets
...
> [!IMPORTANT]  
> Compressed and uncompressed backups can't occur together in a media set. Any edition of [!INCLUDE [sql2008-md](../../includes/sql2008-md.md)] and later versions can read compressed backups. For more information, see [Backup compression (SQL Server)](backup-compression-sql-server.md).

<a id="Overwriting"></a>

## Overwrite backup sets
...
- The existing backups on the media haven't expired. (If `SKIP` is specified, expiration isn't checked.)
  ...
  The expiration date specifies the date that the backup expires and can be overwritten by another backup. You can specify the expiration date when a backup is created. By default, the expiration date is determined by the `media retention` option set with `sp_configure` For more information, see [sp_configure](../system-stored-procedures/sp-configure-transact-sql.md).
...
If you're sure you want to overwrite the existing media (for example, if you know that the backups on the tape are no longer needed), you can explicitly skip these checks.
...
- For restore:
  ...
  For any restore from disk backups and for any online restore, all the media families must be concurrently mounted. For an offline restore from tape backups, you can process the media families from fewer backup devices. Each media family must be processed completely before starting to process another media family. Media families are always processed in parallel, unless they're being restored with a single device.

## Related tasks

### Create a new media set

- [Create a Full Database Backup](create-a-full-database-backup-sql-server.md) (**Back up to a new media set, and erase all existing backup sets** option)
- [BACKUP](../../t-sql/statements/backup-transact-sql.md) (`FORMAT` option)
- <xref:Microsoft.SqlServer.Management.Smo.Backup.FormatMedia%2A>

### Append a new backup to existing media

- [Create a Full Database Backup](create-a-full-database-backup-sql-server.md) (**Append to the existing backup set** option)
- [BACKUP](../../t-sql/statements/backup-transact-sql.md) (`NOINIT` option)

### Overwrite existing backup sets

- [Create a Full Database Backup](create-a-full-database-backup-sql-server.md) (**Overwrite all existing backup sets** option)
- [BACKUP](../../t-sql/statements/backup-transact-sql.md) (`INIT` option)
...
- [backupmediafamily](../system-tables/backupmediafamily-transact-sql.md) (`family_sequence_number` column)
...
- [View the Properties and Contents of a Logical Backup Device (SQL Server)](view-the-properties-and-contents-of-a-logical-backup-device-sql-server.md)
- [RESTORE statements - HEADERONLY](../../t-sql/statements/restore-statements-headeronly-transact-sql.md)

### Read the media header of the media on a backup device

- [RESTORE Statements - LABELONLY](../../t-sql/statements/restore-statements-labelonly-transact-sql.md)
