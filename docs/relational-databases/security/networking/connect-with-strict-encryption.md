...
Starting with [!INCLUDE [sssql25-md](../../../includes/sssql25-md.md)] RC 0 you can connect to an Always On availability group using the `Strict` or `Mandatory` connection encryption type. Your availability group can only enforce encryption if it's based on a Windows Server Failover Cluster. Other types of availability groups don't support strict encryption.
...
Starting with [!INCLUDE [sssql25-md](../../../includes/sssql25-md.md)] RC 0, you can connect to an [Always On failover cluster instance](../../../sql-server/failover-clusters/windows/always-on-failover-cluster-instances-sql-server.md) using the `Strict` or `Mandatory` connection encryption type. To do this, follow these steps: 
