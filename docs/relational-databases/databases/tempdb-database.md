...
<sup>1</sup> Setting `ACCELERATED_DATABASE_RECOVERY` to `ON` in `tempdb` is supported starting with [!INCLUDE[sssql25-md](../../includes/sssql25-md.md)]. In previous versions of [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)], modifying the `ACCELERATED_DATABASE_RECOVERY` option for the `tempdb` database is not allowed.
...
#### Introduced in [!INCLUDE [sssql25-md](../../includes/sssql25-md.md)]
