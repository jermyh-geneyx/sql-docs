---
title: "SQL Server Express LocalDB Header and Version Information"
...
There's no separate header file for the SQL Server Express LocalDB instance API; the LocalDB function signatures and error codes are defined in the Microsoft OLE DB Driver for SQL Server header file (`msoledbsql.h`). To use the LocalDB instance API, you must include the `msoledbsql.h` header file in your project. This article no longer references the SQL Server Native Client header file (`sqlncli.h`).

## LocalDB versioning

The LocalDB installation uses a single set of binaries per major SQL Server version. These LocalDB versions are maintained and patched independently. This means that the user has to specify which LocalDB baseline release (that is, major SQL Server version) they're using. The version is specified in the standard version format defined by the .NET Framework `System.Version` class:

`<major>.<minor>[.<build>[.<revision>]]`

The first two numbers in the version string (`<major>` and `<minor>`) are mandatory. The last two numbers in the version string (`<build>` and `<revision>`) are optional and default to zero if the user leaves them out. This means that if the user specifies only `12.2` as the LocalDB version number, it's treated as if the user specified `12.2.0.0`.
...
## Locate the SQLUserInstance DLL
...
64-bit installations of LocalDB have an extra set of registry keys to allow 32-bit applications running in Windows-32-on-Windows-64 (WOW64) mode to use LocalDB. Specifically, on 64-bit Windows, the LocalDB MSI creates the following registry keys:
...
64-bit programs reading the `Installed Versions` key see values pointing to 64-bit versions of the `SQLUserInstance` DLL, while 32-bit programs (running on 64-bit Windows in WOW64 mode) are automatically redirected to an `Installed Versions` key located under the `Wow6432Node` hive. This key contains values pointing to 32-bit versions of the `SQLUserInstance` DLL.
