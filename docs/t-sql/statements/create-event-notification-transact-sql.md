...
description: CREATE EVENT NOTIFICATION creates an object that sends information about a database or server event to a Service Broker service.
...
The name of the event notification. An event notification name must comply with the rules for [identifiers](../../relational-databases/databases/database-identifiers.md) and must be unique within the scope in which they're created: `SERVER`, `DATABASE`, or *object_name*.
...
The name of the queue to which the event notification applies. *queue_name* can be specified only if `QUEUE` is specified.
...
[!INCLUDE [ssSB](../../includes/sssb-md.md)] includes a message type and contract specifically for event notifications. Therefore, a Service Broker initiating service doesn't have to be created because one already exists that specifies the following contract name: `http://schemas.microsoft.com/SQL/Notifications/PostEventNotification`
...
If an event transaction that activates a notification is rolled back, the sending of the event notification is also rolled back. Event notifications don't fire by an action defined in a trigger when the transaction is committed or rolled back inside the trigger. Because trace events aren't bound by transactions, event notifications based on trace events are sent regardless of whether the transaction that activates them is rolled back.
...
The event transaction that originally started the notification isn't affected by the success or failure of the sending of the event notification.
...
> [!NOTE]  
> In Examples A and B, the GUID in the `TO SERVICE 'NotifyService'` clause (`8140a771-3c4b-4479-8ac0-81008ab17984`) is specific to the computer on which the example was set up. For that instance, it was the GUID for the [!INCLUDE [ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] database.
>
> To copy and run these examples, you need to replace this GUID with one from your computer and [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] instance. As explained in the [Arguments](#arguments) section, you can acquire the '*broker_instance_specifier*' by querying the `service_broker_guid` column of the `sys.databases` catalog view.

### A. Create an event notification that is server scoped
...
### B. Create an event notification that is database scoped
...
### C. Get information about an event notification that is server scoped
...
### D. Get information about an event notification that is database scoped
...
- [sys.server_event_notifications](../../relational-databases/system-catalog-views/sys-server-event-notifications-transact-sql.md)
- [sys.events](../../relational-databases/system-catalog-views/sys-events-transact-sql.md)
- [sys.server_events](../../relational-databases/system-catalog-views/sys-server-events-transact-sql.md)
