...
- Automatic failover in the event of a failure (hardware failures, operating system failures, or application and service failures).
  ...
  > [!IMPORTANT]  
  > In an availability group, automatic failover from an FCI to other nodes within the availability group isn't supported. This means that FCIs and standalone nodes shouldn't be coupled together within an availability group if automatic failover is an important component your high availability solution. However, this coupling can be made for your *disaster recovery* solution.
...
In a production environment, use static IP addresses in conjunction the virtual IP address of a failover cluster instance.
...
1. Client application connection requests are automatically directed to the new active node using the same virtual network name (VNN).
...
- The detailed component diagnostics makes it possible to configure a more flexible failover policy, whereby you can choose what failure conditions trigger failovers and which failure conditions don't.
...
[!INCLUDE [sssql25-md](../../../includes/sssql25-md.md)] RC 0 introduces [TDS 8.0](../../../relational-databases/security/networking/tds-8.md) support, which allows enforcing [TLS 1.3](../../../relational-databases/security/networking/tls-1-3.md) encryption for connections to your failover cluster instances, including from the Windows Server Failover Cluster.
...
The product binaries are installed locally on each node of the FCI, a process similar to [!INCLUDE [ssNoVersion](../../../includes/ssnoversion-md.md)] stand-alone installations. However, during startup, the services aren't started automatically, but managed by WSFC.
...
## SQL Server FCI supported configuration on Windows Server Failover Cluster (WSFC)

Microsoft supports [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] FCIs based on WSFC in the following products:
...
> [!NOTE]  
> For Windows Server 2012 and Windows Server 2012 R2, see [Microsoft support policy for Windows Server failover clusters](/troubleshoot/windows-server/high-availability/microsoft-support-policy-failover-clusters).
...
- A dedicated cluster group with uniquely assigned disk drive letters.
- At least one unique IP address.
- Unique virtual server and instance names within the domain.

### Third-party cluster solution support

[!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] is developed and tested with Microsoft Server Clustering. When using a third-party clustering product, your primary support contact for installation, performance, or cluster behavior issues should be the third-party solution provider. Microsoft offers commercially reasonable support for third-party cluster installations, similar to support for stand-alone [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] deployments.
...
For supported operating systems for [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] Failover Clustering, see [Verify your operating system before installing Failover Clustering](../install/before-installing-failover-clustering.md#OS_Support).

### Mounted drives

The use of mounted drives isn't supported on a cluster that includes a [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] installation. For more information, see [SQL Server support for mounted volumes](../install/before-installing-failover-clustering.md#Hardware).
...
To use CSV with [!INCLUDE [sssql14-md](../../../includes/sssql14-md.md)] or later versions, review the following resources:
...
[!INCLUDE [ssversion2005-md](../../../includes/ssversion2005-md.md)] and later versions can't be migrated to a new domain. You need to uninstall and reinstall the failover cluster components. For more information about how to move a Windows Server cluster from one domain to another, see [Move a Windows Server cluster from one domain to another](/previous-versions/troubleshoot/windows-server/move-server-cluster-to-another-domain).

Before uninstalling [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)], the following steps should be taken:
...
- Don't remove any of the following components: [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] support files, [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] Native Client, Integration Services, or Workstation Components, unless you're rebuilding the entire node.

> [!WARNING]  
> If errors occur during the un-installation process, you might need to rebuild the node to successfully install SQL Server again.
