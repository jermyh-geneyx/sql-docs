...
- You might need to configure scanning software, such as antivirus and antispyware applications, to exclude SQL Server folders and file types. For more information, see [Configure antivirus software to work with SQL Server](/troubleshoot/sql/database-engine/security/antivirus-and-sql-server).
...
## Database Engine Configuration - Server Configuration page
...
## Database Engine Configuration - Data Directories page

Use this page to specify the installation location for the [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE [ssDE](../../includes/ssde-md.md)] program and data files. Based on the type of installation, the supported storage might include local disk, shared storage, or an SMB file server.
...
<sup>1</sup> Although shared disks are supported, we don't recommend their use for a standalone instance of [!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)].
...
- **Initial size (MB)** is the initial size of the `tempdb` log file. The default value is 8 MB (or 4 MB for [!INCLUDE [ssexpress](../../includes/ssexpress-md.md)]). [!INCLUDE [sssql17](../../includes/sssql17-md.md)] introduces a maximum initial file size of 262,144 MB (256 GB). [!INCLUDE [sssql16-md](../../includes/sssql16-md.md)] has a maximum initial file size of 1024 MB. Because `tempdb` is re-created every time SQL Server starts or fails over, specify a size that's close to the size required by your workload for normal operation. To further optimize the creation of **tempdb** during startup, enable [database instant file initialization](../../relational-databases/databases/database-instant-file-initialization.md).
  ...
  > [!NOTE]  
  > `tempdb` uses minimal logging. The `tempdb` log file can't be backed up. It's re-created every time SQL Server starts or when a cluster instance fails over.
...
- **Click here to accept the recommended memory configurations for the SQL Server Database Engine**: Select this check box to accept the calculated recommended memory configurations on this server. If the **Recommended** radio button was selected, setup can't continue without this check box being selected.
...
To make changes to the user roles after the installation program ends, use [Join a Role](../../relational-databases/security/authentication-access/join-a-role.md) or [ALTER ROLE](../../t-sql/statements/alter-role-transact-sql.md).
