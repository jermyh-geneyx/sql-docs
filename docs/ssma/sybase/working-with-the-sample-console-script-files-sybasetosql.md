...
- `ServersConnectionFileSample.xml`:
  ...
  - The user can connect to the required database by changing the values to the required source and target server definitions. In the example provided, all values have been provided as variable values that are available in the `VariableValueFileSample.xml`. All other connection parameters can be removed from the user's working server connection file.

  - For more information, see [Creating the Server Connection Files](creating-the-server-connection-files-sybasetosql.md).

- `VariableValueFileSample.xml`:

  All variables that have been used in the sample console script files and `ServersConnectionFileSample.xml` have been collated in this file. To execute the sample console scripts the user has to replace the sample variable values with user defined ones and pass this file as an extra command line argument along with the script file.

  For more information, see [Creating Variable Value Files](creating-variable-value-files-sybasetosql.md).
...
- `AssessmentReportGenerationSample.xml`:
  ...
  In the `generate-assessment-report` command the user has to mandatorily change the variable value (refer `VariableValueFileSample.xml`) in the `object-name` attribute to the database name being in use by the user. Depending on the kind of object specified, the `object-type` value must also be changed.
  ...
  For more information, see [Generating Reports](generating-reports-sybasetosql.md).
...
- `SqlStatementConversionSample.xml`:
  ...
  In the `convert-sql-statement` command the user has to mandatorily change the variable value (refer `VariableValueFileSample.xml`) in the `context` attribute to the database name that's being in use by the user. The user must change the `sql` attribute value to the source database `sql` command that they require to be converted.

  The user can also provide sql files to be converted. This is shown in the `convert-sql-statement` command's Example 4 of the sample console script file.
...
- `ConversionAndDataMigrationSample.xml`:
  ...
  This sample enables the user to perform an end to end migration from conversion to data migration. The list of mandatory attribute values that they have to change is listed as follows:

  - **Command name**: `map-schema`
  ...
  - **Command name**: `synchronize-target`
...
- [Creating the Server Connection Files](creating-the-server-connection-files-sybasetosql.md)
