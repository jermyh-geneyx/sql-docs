...
Specifies the instance of [!INCLUDE [ssNoVersion](../includes/ssnoversion-md.md)] where the operation runs. The server instance to specify depends on which log-shipping operation is being specified. For `-backup`, *instance_name* must be the name of the primary server in a log shipping configuration. For `-copy` or `-restore`, *instance_name* must be the name of a secondary server in a log shipping configuration.
...
| `3` (default) | Output informational messages, warnings, and error-handling messages. |
| `4` | Output all debugging and tracing messages. |

#### -logintimeout *timeout_value*

Specifies the amount of time allotted for attempting to sign in to the server instance before the attempt times out. The default is 15 seconds. *timeout_value* is **int**.
...
Specifies extra connectivity options in the form of key value pairs.
...
| `ServerCertificate` | Path on the filesystem to the server certificate. This option has a maximum length of 260 characters. |
| `HostNameInCertificate` | Hostname override for the certificate. This option has a maximum length of 255 characters. |
...
| --- | --- | --- |
| `-backup` | Requires read/write access to the backup directory. | Requires the same permissions as the `BACKUP` statement. For more information, see [BACKUP](../t-sql/statements/backup-transact-sql.md). |
...
| `-restore` | Requires read/write access to the copy directory. | Requires the same permissions as the `RESTORE` statement. For more information, see [RESTORE Statements](../t-sql/statements/restore-statements-transact-sql.md). |
