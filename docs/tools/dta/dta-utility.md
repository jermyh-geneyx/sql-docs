...
The default storage size doesn't include nonclustered indexes and indexed views.
...
But if you use the same workload, and use the following **dta** command that uses the `-d` argument, a recommendation is generated:
...
Specifies the name of the table or file where **dta** records events that it couldn't tune. The table is created on the server where the tuning is performed.
...
Permits **dta** to overwrite an existing output file. If an output file with the same name already exists and `-F` isn't specified, **dta** returns an error. You can use `-F` with `-of`, `-or`, or `-ox`.
...
Specifies that columnstore indexes be considered for new recommendations. DTA considers both clustered and nonclustered columnstore indexes. For more information, see
...
Specifies that **dta** only considers dropping existing physical design structures. No new physical design structures are considered. When this option is specified, **dta** evaluates the usefulness of existing physical design structures and recommends dropping seldom used structures. This argument takes no values. It can't be used with the `-fa`, `-fp`, or `-fk ALL` arguments

#### -ID *session_ID*

Specifies a numerical identifier for the tuning session. If not specified, then **dta** generates an ID number. You can use this identifier to view information for existing tuning sessions. If you don't specify a value for `-ID`, then a session name must be specified with `-s`.

#### -ip

Specifies that the plan cache is used as the workload. The top 1,000 plan cache events for explicitly selected databases are analyzed. This value can be changed using the `-n` option.

#### -iq

Specifies that the Query Store is used as the workload. The top 1,000 events from the Query Store for explicitly selected databases are analyzed. This value can be changed using the `-n` option. For more information, see [How Query Store collects data](../../relational-databases/performance/how-query-store-collects-data.md) and
[Tuning Database Using Workload from Query Store with the Database Engine Tuning Advisor](../../relational-databases/performance/tuning-database-using-workload-from-query-store.md).

> **Applies to**: [!INCLUDE [sssql16-md](../../includes/sssql16-md.md)] and later versions.

#### -if *workload_file*

Specifies the path and name of the workload file to use as input for tuning. The file must be in one of these formats: `.trc` (SQL Server Profiler trace file), `.sql` (Transact-SQL file), or `.log` ([!INCLUDE [ssNoVersion](../../includes/ssnoversion-md.md)] trace file). Either one workload file or one workload table must be specified.
...
In this case, it's important to specify an unlimited tuning time (`-A 0`). Otherwise, Database Engine Tuning Advisor assumes an 8 hour tuning time by default.
...
In this case, DTA uses Query Store as the source of workload and only consider queries that have executed with the past 48 hours.
...
You can use `-F` with this option. Make sure that the file name is unique, especially if you're also using `-or` and `-ox`.
...
You can use `-F` with this option. Make sure that the file name is unique, especially if you're also using `-of` and `-ox`.
...
You can use `-F` with this option. Make sure that the file name is unique, especially if you're also using `-of` and `-or`.

#### -P *password*

Specifies the password for the login ID. If this option isn't used, **dta** prompts for a password.
...
This argument is an alternative to entering a table list at the command prompt (`-Tl`). Don't use a table list file (`-Tf`) if you're using `-Tl`. If both arguments are used, **dta** fails and returns an error.

If the `-Tf` and `-Tl` arguments are omitted, all user tables in the specified databases are considered for tuning.

#### -Tl *table_list*

Specifies at the command prompt a list of tables to be tuned. Place commas between table names to separate them. If only one database is specified with the `-D` argument, then table names don't need to be qualified with a database name. Otherwise, the fully qualified name in the format: *database_name.schema_name.table_name* is required for each table.
...
This example limits the total database size, which includes the raw data and the extra indexes, to 3 gigabytes (GB) (`-B 3000`) and directs the output to `d:\result_dir\script1.sql`. It runs for no more than 1 hour (`-A 60`).
...
- The number of rows in `Store` are assumed to be the current number of rows in the table.
  ...
  There can be one or more spaces between the number of rows count and the preceding table name in the *table_list_file*.
