...
To toggle the gathering of client statistics, when the Transact-SQL Editor is open, on the **Data** menu, point to **Transact-SQL Editor**, select **Execution Settings** and **Include Client Statistics**. Alternatively, select the **Include Client Statistics** button (the fifth one from the right) on the Transact-SQL Editor toolbar, or by right-clicking in the Transact-SQL editor and then selecting **Execution Settings** and **Include Client Statistics**. In order to gather statistics for a query, you have to turn on this feature before executing it.

If you turned on client statistics, the **Statistics** tab appears next to the **Message** tab upon query execution. If you turned off client statistics, the **Statistics** tab doesn't appear. Statistics from successive query executions are listed along with the average values.

For more information on the statistics collected, see [Query Window Statistics pane](https://www.microsoft.com/download/details.aspx?id=51958) and [Client Statistics Tab](/previous-versions/visualstudio/visual-studio-2010/aa833205(v=vs.100)#client-statistics-tab).
