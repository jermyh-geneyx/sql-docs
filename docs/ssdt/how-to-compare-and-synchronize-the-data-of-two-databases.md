...
> [!NOTE]  
> *Database projects* and `.dacpac` or `.bacpac` packages can't be the source or target in a data comparison.
...
> [!NOTE]  
> You can also compare the *schema* of two databases or of two versions of the same database. For more information, see [Schema comparison overview](../tools/sql-database-projects/concepts/schema-comparison.md).
...
1. Select the check boxes for the tables and views that you want to compare. Optionally, expand the nodes for database objects, and then select the check boxes for columns within those objects that you want to compare.
   ...
   > [!NOTE]  
   > Tables and views must meet two criteria to appear in the listing. First, the schemas of the objects must match between the source and target databases. Second, only tables and views that have a primary key, a unique key, a unique index, or a unique constraint appears in the list. If no tables or views meet both criteria, the list is empty.
...
1. (Optional) Do one or both of the following options:
...
1. In the table of the Data Compare window, select a row.
   ...
   The details pane shows results for the records in the database object that you selected. Records are grouped by status onto tabs, which you can use to specify the data that is propagated from the source to the target.
