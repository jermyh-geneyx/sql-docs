...
1. SQL Server Data Tools (SSDT) attempts to build the project first. If there's no build error, a **Snapshot** folder is created in **Solution Explorer**. Inside this folder, SSDT creates a `.dacpac` file using the name format of `<Project Name>_YYYYMMDD_HH-MM-SS.dacpac`.

1. Right-click the `.dacpac` file and select **Rename**. Change the default file name to `TradeDev1.dacpac`.

1. Right-click the **GetProductsBySupplier** function in **Solution Explorer** and select **Delete** to remove it from the project.

1. Follow the previous steps to create a new snapshot called `TradeDev2.dacpac`.
...
1. In the **Import Data-tier Application** dialog box, select **Browse** to select `TradeDev1.dacpac` to be used as the source of the import.

   The **Target project** section has been disabled, since the current project is the default target. Select **Start** to start the import.

1. Select **Finish** in the **Summary** page. In **Solution Explorer**, notice that the deleted table has been restored to the project.

   > [!WARNING]  
   > The import snapshot imports all database entities in the snapshot schema to the project. This might create duplicate entities as a result. For example, each of the tables and view now contains another copy of itself named <ObjectName_1>. Right-click each of these duplicate objects in **Solution Explorer** and select **Delete** to remove it from the project.

## Compare snapshots

1. Right-click `TradeDev1.dacpac` in Solution Explorer and select **Schema Compare**. The **Schema Compare** window opens.

1. Use the **Data-tier Application File** options to set the source and target schemas. Make sure that the **Source Schema** is set to `TradeDev1.dacpac` in **Data-tier Application File**, and the **Target Schema** is set to `TradeDev2.dacpac`.

1. Select **OK** to start the compare. The deleted function is being highlighted as a difference between the old and new snapshot.
