...
|ReportFolder|**String**|The full path to the folder containing the report. This item doesn't include the report server URL.|  
|ReportName|**String**|The name of the report as it is stored in the report server database.|  
|ReportServerUrl|**String**|The URL of the report server on which the report is run.|  
...
|OverallPageNumber|**Integer**|The page number of the current page for the entire report. This value isn't affected by **ResetPageNumber**.<br /><br /> **OverallPageNumber** can only be used in an expression in a page header or page footer.|  
|OverallTotalPages|**Integer**|The total number pages for the entire report. This value isn't affected by **ResetPageNumber**.<br /><br /> **OverallTotalPages** can only be used in an expression in a page header or page footer.|  
