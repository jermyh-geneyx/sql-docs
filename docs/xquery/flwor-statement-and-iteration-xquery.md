...
description: Learn about for, let, where, order by, and return clauses that make up the FLOWR iteration syntax in XQuery.
author: rothja
ms.author: jroth
...
- The [path expression](path-expressions-xquery.md), `//AWMI:root/AWMI:Location[1]/AWMI:step`, generates the input sequence. This sequence is the sequence of the <`step`> element node children of the first <`Location`> element node.

- The optional predicate clause, `where`, isn't used.

- The `return` expression returns a string value from the <`step`> element.

The [string function (XQuery)](data-accessor-functions-string-xquery.md) is used to retrieve the string value of the <`step`> node.
...
In [!INCLUDE [ssNoVersion](../includes/ssnoversion-md.md)], heterogeneous sequences aren't allowed. Specifically, sequences that contain a mixture of atomic values and nodes aren't allowed.
...
- The [data function (XQuery)](data-accessor-functions-data-xquery.md) is used to extract the value of each attribute so they're added to the resulting XML as text nodes instead of as attributes.
...
1. If the `where` expression returns one simple Boolean type value, that value is the effective Boolean value.

1. If the `where` expression returns a sequence that contains at least one node, the effective Boolean value is True.

1. Otherwise, it raises a static error.
...
The following query is similar, except that it's specified against the Instructions column, typed **xml** column, of the `ProductModel` table. [XML construction (XQuery)](xml-construction-xquery.md) is used to generate the XML that you want.
...
The [Atomization (XQuery)](atomization-xquery.md) process retrieves the atomic value of the <`number`> elements before passing it to `order by`. You can write the expression by using the `data()` function, but that isn't required.
...
- The empty least, empty greatest, and collation keywords on `order by` aren't supported
