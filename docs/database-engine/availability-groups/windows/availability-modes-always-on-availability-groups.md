...
*Configuration only mode* applies to availability groups that aren't on a Windows Server Failover Cluster. A replica in configuration only mode doesn't contain user data. In configuration only mode, the replica master database stores availability group configuration metadata. For more information see [Availability group with configuration only replica](../../../linux/sql-server-linux-availability-group-ha.md).
...
1. The primary replica writes the record to the transaction log and concurrently sends the log record to the secondary replicas.
   ...
   Once a log record is written to the transaction log of the primary database, the transaction can be undone only if there's a failover to a secondary that did not receive the log.
