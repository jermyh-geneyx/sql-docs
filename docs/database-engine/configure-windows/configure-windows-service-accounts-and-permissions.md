...
[!INCLUDE [ssISnoversion](../../includes/ssisnoversion-md.md)] might include additional services for scale-out deployments. For more information, see [Walkthrough: Set up Integration Services (SSIS) Scale Out](../../integration-services/scale-out/walkthrough-set-up-integration-services-scale-out.md).
...
The [Local audit for SQL Server usage and diagnostic data collection (CEIP)](../../sql-server/usage-and-diagnostic-data-in-local-audit.md) sends [telemetry](../../sql-server/sql-server-privacy.md) data back to Microsoft.
...
If you change service accounts for any SQL service by using other means, it can lead to unexpected behavior or errors. For example, if you change SQL Agent service account to a domain account using Windows services applet, you might notice that SQL agent jobs that use Operating System (Cmdexec), Replication, or SSIS job steps might fail with an error such as:
...
- **Group-managed service accounts**
  ...
  To use a gMSA for SQL Server 2014 or later, the operating system must be Windows Server 2012 R2 or later. Servers with Windows Server 2012 R2 require [KB 2998082](https://support.microsoft.com/help/2998082) applied so that the services can sign in without disruption immediately after a password change.
...
For more information on managed service accounts and virtual accounts, see the **Managed service account and virtual account concepts** section of [Service Accounts Step-by-Step Guide](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/dd548356(v=ws.10)) and [Managed Service Accounts Frequently Asked Questions (FAQ)](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/ff641729(v=ws.10)).
...
In most cases, when initially installed, the [!INCLUDE [ssDE](../../includes/ssde-md.md)] can be connected to by tools such as [!INCLUDE [ssManStudioFull](../../includes/ssmanstudiofull-md.md)] installed on the same computer as SQL Server. SQL Server Setup doesn't open ports in the Windows Firewall. Connections from other computers might not be possible until the [!INCLUDE [ssDE](../../includes/ssde-md.md)] is configured to listen on a TCP port, and the appropriate port is opened for connections in the Windows Firewall. For more information, see [Configure the Windows Firewall to allow SQL Server access](../../sql-server/install/configure-the-windows-firewall-to-allow-sql-server-access.md).
...
<a id="Review_additional_considerations"></a>
...
> [!IMPORTANT]  
> Before you upgrade SQL Server, enable SQL Server Agent and verify the required default configuration: the SQL Server Agent service account is a member of the SQL Server **sysadmin** fixed server role.
...
- During upgrade of [!INCLUDE [ssVersion2005](../../includes/ssversion2005-md.md)] to [!INCLUDE [sssql19-md](../../includes/sssql19-md.md)] setup configures the SQL Server instance in the following way:
  ...
  - The local Windows group for services is renamed from `SQLServer2005MSSQLUser$<computer_name>$<instance_name>` to `SQLServerMSSQLUser$<computer_name>$<instance_name>`. File locations for migrated databases have Access Control Entries (ACEs) for the local Windows groups. The file locations for new databases have ACEs for the per-service SID.
...
- For a SQL Server failover cluster instance, the ACEs for the domain account configured for the service are retained.
...
If the service must interact with network services, access domain resources like file shares or if it uses linked server connections to other computers running SQL Server, you might use a minimally privileged domain account. Many server-to-server activities can be performed only with a domain user account. A domain administrator should precreate this account in your environment.
...
The local service account isn't supported for the SQL Server or SQL Server Agent services. Local Service isn't supported as the account running those services because it's a shared service and any other services running under local service would have system administrator access to SQL Server.

The actual name of the account is `NT AUTHORITY\LOCAL SERVICE`.

<a id="Network_Service"></a>
...
<a id="Identify_instance_aware_and_unaware"></a>
...
Instance-aware services in SQL Server include:
...
Instance-unaware services in SQL Server include:

- [!INCLUDE [ssISnoversion](../../includes/ssisnoversion-md.md)]
- SQL Server Browser
- SQL Writer

<a id="Localized_service_names"></a>
