...
- As of [!INCLUDE [sssql22-md](../../includes/sssql22-md.md)] CU 1, *Direct Write* is the default behavior when the hybrid buffer pool is combined with [persisted log buffer](../../relational-databases/databases/add-persisted-log-buffer.md). This should improve performance for almost all workloads, but there's always a chance of regression and the CU should be tested thoroughly before being applied. If you experience regression due to this behavior change, you can revert to the previous behavior using the start-up [trace flag 898](../../t-sql/database-console-commands/dbcc-traceon-trace-flags-transact-sql.md#tf898).
